...
static ERL_NIF_TERM create_test_resource(ErlNifEnv *env, int argc,
  ...
  int size = enif_get_string(env, argv[1], resource->text, resource->text_size(),
                             ERL_NIF_LATIN1);
  if (size <= 0)
...
static ERL_NIF_TERM resource_to_tuple(ErlNifEnv *env, TestResource &resource) {
  return enif_make_tuple2(env, enif_make_int64(env, resource.id),
                          enif_make_string(env, resource.text, ERL_NIF_LATIN1));
}

static ERL_NIF_TERM use_test_resource(ErlNifEnv *env, int argc,
                                      const ERL_NIF_TERM argv[]) {
...
static ERL_NIF_TERM free_test_resource(ErlNifEnv *env, int argc,
                                       const ERL_NIF_TERM argv[]) {
...
ERL_NIF_INIT(Elixir.NifSampleCpp.Nif.SampleResource, nif_funcs, &load, &reload,
             &upgrade, &unload)
